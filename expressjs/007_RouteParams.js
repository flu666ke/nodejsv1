...
// В маршрутах допустимо задавать параметры, для этого необходимо в маршруте определить сегмент, в котором будет находиться параметр
// передачи параметра в сегменте, необходимо перед именем параметра установить двоеточие (:)\
...
app.get('/category/:categoryId', function (request, response) {
    // для получения параметра из сегмента используется свойство params объекта request

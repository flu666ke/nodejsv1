...
// open(filename, flag, callback)
    ...
    // flag - флаг, который указывает каким образом открывать файлы
        ...
        //w - открыть для записи. Если файл не существует, он будет создан. Если файл существует, его содержимое будет о
        //w+ - открыть для чтения и записи. Если файл не существует, он будет создан. Если файл существует, его содержимое
        //a - открыть для записи в конце файла. Если файл не существует, он будет создан;
        //а+ - открыть для чтения и записи в конце файла. Если файл не существует, он будет создан
...
fs.open('demofile.txt', 'w+', function(err, fd){
    ...
    if(err) {
        ...
        // Метод write - альтернатива методу writeFile; позволяет записать данные в файл и принимает такие аргументы:
            ...
            // callback - функция, принимающая аргументы err, written, stirng
       
            fs.write(fd, 'This is the file content!', {encoging: 'utf-8'}, function(err, written, string){
        ...
        // Метод read позволяет читать данные из файла, принимает аргументы:
            // fd - дескриптор файла
            // buffer - буфер, в который будут помещеные прочитанные данные
            // offset, length - определяет часть буфера, которую следует записать в файл
